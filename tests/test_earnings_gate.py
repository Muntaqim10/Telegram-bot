...
def in_window(today: str, earnings: str, expiration: str) -> bool:
    """The pipeline's test: is the print inside the contract's life?"""
    d = datetime.date.fromisoformat
    return d(today) <= d(earnings) <= d(expiration)


class TestTheWindow:
    def test_the_lulu_case(self):
        """Signalled 2026-09-03, 7-14 DTE contract, print that evening."""
        assert in_window("2026-09-03", "2026-09-03", "2026-09-18") is True

    def test_a_print_after_expiry_is_not_our_problem(self):
        assert in_window("2026-09-04", "2026-09-25", "2026-09-18") is False

    def test_a_print_already_behind_us_is_not_either(self):
        assert in_window("2026-09-04", "2026-09-03", "2026-09-18") is False

    def test_earnings_on_expiry_day_still_counts(self):
        """The contract is alive that morning, so the gap still lands on it."""
        assert in_window("2026-09-04", "2026-09-18", "2026-09-18") is True

    def test_earnings_today_counts(self):
        assert in_window("2026-09-04", "2026-09-04", "2026-09-18") is True
...
class TestItFailsOpen:
    ...
    def test_no_earnings_date_means_no_block(self):
        next_earnings = None
        earnings_in_window = False
        if next_earnings:  # the pipeline's guard
            earnings_in_window = True
        assert earnings_in_window is False

    def test_an_unparseable_date_means_no_block(self):
